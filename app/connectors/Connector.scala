...
trait Connector {
  ...
  def post[T: Tag, R](
    ...
    body: T,
    additionalHeaders: Seq[(String, String)] = List.empty
  )(implicit ec: ExecutionContext, hc: HeaderCarrier, writes: BodyWritable[T], reads: HttpReads[R]): Future[R] =
    transform(httpClient.post(url"$url"), additionalHeaders).withBody[T](body).execute[R]
