...
class LanguageSwitchControllerISpec extends IntegrationSpec {
  ...
  private def switchLanguageRoute(lang: String): String =
    LanguageSwitchController.switchToLanguage(lang).url
  ...
  "LanguageSwitch Controller" when {
    ...
    "no referer in header" must {
      "redirect to login continue url" in {
        ...
        redirectLocation(result).value mustEqual fakeApplication().configuration
          .getOptional[String]("urls.loginContinue")
          .getOrElse(throw new Exception("Missing continue login url configuration"))
      }
    }
  }
}
