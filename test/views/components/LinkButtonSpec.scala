...
class LinkButtonSpec extends UnitViewSpec {

  private val linkPartial = instanceOf[link_button]

  private def createComponent(): Appendable = linkPartial("signOut.link", ChoiceController.onPageLoad)(messages)

  "linkButton" should {
    "display a link with correct href" in {
      val element = createComponent().getElementsByClass("govuk-button").first()
